require 'app/things/thing.rb'

require 'app/things/grids/highlighter.rb'

require 'app/geometry/unit_converter.rb'
...
class Grid < Thing
	...
	private
	attr_writer :target_location, :target_cell, :unit_length, :unit_converter, :highlighter, :unit_length

	# @param location (see Extant#initialize)
	# @param size_in_units [Array<Integer>] The Width and Height of the object measured by the Grid's cell unit.
	# @example Grid.new([50, 100], [7, 4], 128)
	def initialize(location, size_in_units = [16, 16], unit_length = 16)
		...
		self.unit_converter = Converter.new(unit_length)
	...
	attr_reader :unit_length, :target_cell, :target_location, :highlighter, :unit_converter
	...
	# Used to inform the Grid that a section of it is being targeted. It takes the target location and converts it into 
	...
	def calculate_literal_location(location)
		int_x = unit_converter.calculate_target_location(location[0], x)
		int_y = unit_converter.calculate_target_location(location[1], y)
	...
	def calculate_cell_location(location)
		cell_x = unit_converter.calculate_target_cell(location[0], x)
		cell_y = unit_converter.calculate_target_cell(location[1], y)
	...
	# @return [Array<Integer>]
	def apex_int
		[unit_converter.calculate_target_location(right_side-1, x), unit_converter.calculate_target_location(top_side-1, y)]
	end

	# This first notifies the Grid that a certain point has been targeted. 
	...
	# Diagnostic values that can be rendered to the screen with the Grid itself.
	# @return [Hash]
	def diagnostics
		[
			{
